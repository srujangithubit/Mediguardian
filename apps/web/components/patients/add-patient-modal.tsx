...
export function AddPatientModal({ familyId, onClose, onSuccess }: { familyId: string, onClose: () => void, onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await api.post(`/families/${familyId}/members`, data);
      onSuccess();
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="glass w-full max-w-lg rounded-3xl overflow-hidden border border-white/10 shadow-2xl animate-in fade-in zoom-in duration-200">
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
          <h2 className="text-xl font-bold">Add Dependent</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input required name="fullName" type="text" className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent" placeholder="John Doe" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-text/60 mb-1">Date of Birth</label>
              <input name="dateOfBirth" type="date" className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent" />
            </div>
            <div>
              <label className="block text-xs font-medium text-text/60 mb-1">Gender</label>
              <select name="gender" className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent appearance-none">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select name="bloodGroup" className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent appearance-none">
                <option value="">Select...</option>
            ...
            <div>
              ...
              <input name="allergies" type="text" className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent" placeholder="Peanuts, Penicillin" />
            </div>
          </div>

          <div>
            <label className="block text-xs font-medium text-text/60 mb-1">Medical Notes</label>
            <textarea name="medicalNotes" rows={3} className="w-full bg-background/50 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-accent resize-none" placeholder="Any underlying conditions or special care notes..."></textarea>
          </div>
          ...
          <div className="pt-4 flex gap-3">
            ...
            <button type="submit" disabled={loading} className="flex-1 py-3 rounded-xl bg-accent text-white font-medium hover:bg-accent-secondary transition-colors disabled:opacity-50 flex justify-center items-center">
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Add Dependent'}
            </button>
          </div>
        </form>
      </div>
